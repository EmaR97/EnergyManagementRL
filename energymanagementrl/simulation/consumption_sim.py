...
class ConsumptionSim(EnergySim):
    ...
    def __init__(self, power_series: list[float], max_step: int = None, max_24h: int = None,
        ...
        self.energy_samples = [get_smoothed_history(sample, self.forecast_range) for sample in self.energy_series]
...
def get_smoothed_history(new_point, _range):
    global buffer
    if not len(buffer):
        buffer = [new_point] * (day * 2)
