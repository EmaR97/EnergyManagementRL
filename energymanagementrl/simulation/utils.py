min5 = 1 / 12
day = 24 * 12
week = day * 7
month = week * 4
full_period = month * 5

from scipy.ndimage._filters import _gaussian_kernel1d, correlate1d


class SmoothedHistory:
    def __init__(self, sigma, steps):
        self.sigma = sigma
        self.steps = steps
        self.buffer = []
        self.kernel = self._create_gaussian_kernel(sigma, steps)

    def _create_gaussian_kernel(self, sigma, steps):
        return _gaussian_kernel1d(sigma, 0, int(4 * float(sigma) + 0.5))[::-1]

    def get_smoothed_history(self, new_point, _range):
        if not self.buffer:
            self.buffer = [new_point] * self.steps
        self.buffer.append(new_point)
        self.buffer.pop(0)
        smoothed = correlate1d(self.buffer, self.kernel, -1, None, "reflect", .0, 0)
        return [smoothed[i] for i in _range]
